...
def _service_line_issues(
    ...
    """Человекочитаемые причины по строкам талона (нумерация с 1 — порядок строк в выгрузке по талону)."""
    col_code = "Код услуги"
    issues: list[str] = []
    for i, r in enumerate(trows, start=1):
        ...
        if not SERVICE_CODE_RE.match(code):
            issues.append(
                f"стр.{i}: неверный формат кода «{code}» "
                f"(ожидается Axx.xx.xxx или Bxx.xx.xxx, при необходимости с уточнением вида .xxx)"
            )
            continue
        if not _service_code_accepted_by_reference(code, allowed_codes):
            issues.append(
                f"стр.{i}: код «{code}» нет в справочнике dn_service (catalog=global)"
            )
    return issues
...
def _ticket_service_lines_match_reference(
    ...
    """Нет «левых» строк услуг: верный формат (в т.ч. уточнение .xxx после базы) и код в справочнике —
    либо полный код, либо база Axx.xx.xxx / Bxx.xx.xxx при уточняющем хвосте.
    Строки без кода в выгрузке (прочерк и т.п.) не учитываются — талон может считаться корректным.
    """
...
def build_report(
    ...
    for tid, trows in ticket_rows.items():
        ...
        if use_dx:
            ...
            if _ticket_service_lines_match_reference(trows, allowed_codes):
    ...
    for tid, trows in sorted(ticket_rows.items(), key=lambda x: x[0]):
        issues = _service_line_issues(trows, allowed_codes)
        if not issues:
            continue
        first = trows[0]
        ...
        tickets_service_mismatch.append(
            {
                ...
                "диагноз": _norm_code(first.get(col_ds1)) or "(без диагноза)",
        ...
        for cat, agg in sorted(
            ...
            wsc1.append(
                [
                    cat,
                    agg["талонов"],
                    round(float(agg["сумма_талонов"]), 2),
                    int(agg.get("талонов_услуги_ок", 0)),
                    round(float(agg.get("сумма_услуги_ок", 0.0)), 2),
                ]
            )
