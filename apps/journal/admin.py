...
class CitizenAppealAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if getattr(self.instance, 'appeal_id', None):
...
class CitizenAppealAdmin(ModelAdmin):
    ...
    change_list_template = "admin/journal/citizenappeal/change_list_with_report.html"
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        # Сортируем по возрастанию, чтобы первый год был самым ранним (например, 2024)
        distinct_years_list = sorted(distinct_years)

        if distinct_years_list:
            year_str = request.GET.get("year")
            if year_str and year_str.isdigit() and int(year_str) in distinct_years_list:
                selected_year = int(year_str)
            else:
                selected_year = distinct_years_list[0]  # самый ранний год
        else:
            selected_year = None

        qs = base_qs.filter(registration_date__year=selected_year) if selected_year is not None else base_qs.none()

        # Теперь формируем данные отчёта по месяцам
        month_names = [
        ...
        for month_idx in range(1, 13):
            month_qs = qs.filter(registration_date__month=month_idx)
            # Ниже пример подсчётов (замените полями/фильтрами, которые вам нужны)
            count_priyom = month_qs.filter(retrieved_with_applicant=True).count()
            count_zayav = month_qs.count()
            count_supported_measures = month_qs.filter(supported_measures_taken=True).count()
            count_rebuke = month_qs.filter(supported_measures_taken_rebuke=True).count()
            count_comment = month_qs.filter(supported_measures_taken_comment=True).count()
            count_deprivation = month_qs.filter(supported_measures_taken_deprivation_payments=True).count()
            count_meeting = month_qs.filter(supported_measures_taken_holding_meeting=True).count()
            count_other = month_qs.filter(supported_measures_taken_other=True).count()
            count_supported_no_measures = month_qs.filter(supported_without_measures=True).count()
            count_not_supported = month_qs.filter(not_supported=True).count()
            count_explained = month_qs.filter(explained=True).count()
            count_oral_answer = month_qs.filter(retrieved_on_place=True).count()
            count_written_answer = month_qs.filter(answer_author=True).count()

            report_rows.append({
                "month_name": month_names[month_idx - 1],
                "count_priyom": count_priyom,
                "count_zayav": count_zayav,
                "count_supported_measures": count_supported_measures,
                "count_rebuke": count_rebuke,
                "count_comment": count_comment,
                "count_deprivation": count_deprivation,
                "count_meeting": count_meeting,
                "count_other": count_other,
                "count_supported_no_measures": count_supported_no_measures,
                "count_not_supported": count_not_supported,
                "count_explained": count_explained,
                "count_oral_answer": count_oral_answer,
                "count_written_answer": count_written_answer,
            })

        if extra_context is None:
            extra_context = {}
        extra_context["distinct_years"] = distinct_years_list
        extra_context["selected_year"] = selected_year
        extra_context["report_rows"] = report_rows
