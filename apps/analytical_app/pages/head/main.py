...
cards_row_3 = dbc.Row(
    [
        ...
        dbc.Col(create_card(11, type_page, "Удаление ЭМД", "Управление заявками на удаление ЭМД")),
        dbc.Col(create_card(12, type_page, "Школы здоровья (цель 307)", "Талоны 307, школа по справочнику, периодичность 3 года, план ИСЗЛ.")),
    ],
    className="row-cols-1 row-cols-md-4 g-4 mb-4 align-items-stretch"
)
...
@app.callback(
    ...
    [Input(f'open-report-{i}-{type_page}', 'n_clicks') for i in range(1, 13)],
...
def navigate_pages(*n_clicks):
    ...
    route_map = {
        ...
        11: f"/{main_link}/delete-emd",
        12: f"/{main_link}/health_schools",
    }
