...
from apps.analytical_app.pages.head.dispensary.reproductive.query import sql_query_reproductive, \
    sql_query_reproductive_building, sql_query_reproductive_building_department
from apps.analytical_app.query_executor import engine

type_page = "tab1-dr"

reproductive_dr1 = html.Div(
    [
        dbc.Row(
            dbc.Col(
                dbc.Card(
...
def update_table(n_clicks, selected_period, selected_year, inogorodniy, sanction,
    ...
    columns2, data2 = TableUpdater.query_to_df(
        engine,
        sql_query_reproductive_building(
            selected_year,
            ', '.join([str(month) for month in range(selected_period[0], selected_period[1] + 1)]),
            inogorodniy,
            sanction,
