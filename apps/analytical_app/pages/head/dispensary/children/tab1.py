...
from apps.analytical_app.pages.head.dispensary.children.query import sql_query_dispensary_children, \
    sql_query_dispensary_building_department_children, sql_query_dispensary_building_children
from apps.analytical_app.query_executor import engine

type_page = "tab1-dc"

children_dn1 = html.Div(
    [
        dbc.Row(
            dbc.Col(
                dbc.Card(
...
def update_table(n_clicks, selected_period, selected_year, inogorodniy, sanction,
    ...
    columns3, data3 = TableUpdater.query_to_df(
        engine,
        sql_query_dispensary_building_department_children(
            selected_year,
            ', '.join([str(month) for month in range(selected_period[0], selected_period[1] + 1)]),
            inogorodniy,
            sanction,
