...
from apps.analytical_app.pages.web_oms.status_talon.query import sql_query_cel_dia
...
def update_dropdown(selected_value):
    sql_query_dia = """
        SELECT DISTINCT SUBSTRING(main_diagnosis, 1,
                                  POSITION(' ' IN main_diagnosis) -
                                  1) as "Диагноз"
        FROM data_loader_omsdata
        union all
        select 'БСК'
