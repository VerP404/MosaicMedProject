...
from apps.MosaicMed.generate_pages.constants import status_groups, months_labels
from apps.MosaicMed.pages.economic_reports.by_doctors_dispensary.query import (
 sql_query_by_doctor_dispensary_reproductive_f3,
    sql_query_by_doctor_dispensary_reproductive_f2, sql_query_by_doctor_dispensary_reproductive_f1
)
...
tab3_layout_by_doctor_dispensary = html.Div(
    [
        ...
        card_table(f'result-table1-{type_page}', "Форма 1: по врачам, закрывшим карту", 10),
        card_table(f'result-table2-{type_page}', "Форма 2: по услугам", 10),
        card_table(f'result-table3-{type_page}', "Форма 3: по врачам в услугах", 10),
    ],
    style={"padding": "0rem"}
)
...
def update_table(n_clicks, selected_period, selected_status):
    ...
    columns1, data1 = TableUpdater.query_to_df(engine, sql_query_by_doctor_dispensary_reproductive_f1(sql_cond), bind_params)
    columns2, data2 = TableUpdater.query_to_df(engine, sql_query_by_doctor_dispensary_reproductive_f2(sql_cond), bind_params)
    columns3, data3 = TableUpdater.query_to_df(engine, sql_query_by_doctor_dispensary_reproductive_f3(sql_cond), bind_params)
